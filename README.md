...
| `COUCH_PRELOAD_DBS`                                              | Comma-separated DB names to warm on boot (ensure `_design/acl` + ACL follower)                                                                                                                                                                                                                                                            |
| `COUCH_PRELOAD_DB_INCLUDE`                                       | Opt-in DB name patterns (exact or `/regex/flags`) matched against admin `GET /_all_dbs` at boot. **Union** with `COUCH_PRELOAD_DBS` when both set. System DBs are never selected by patterns. Results are still filtered by `ACL_DB_INCLUDE` / `ACL_DB_EXCLUDE`. Empty/unset = lazy ensure. Example: `COUCH_PRELOAD_DB_INCLUDE=/^data-/`. |
...
| `PROFILE`                                                        | Opt-in request phase profiling (`auth` / `acl` / `aclMiss` / `upstream` / `filter`) plus scrapeable process memory / ACL+session resource sizes on `GET /_couch-auth-proxy/profile`. Also exposes `POST .../profile/reset` and optional `POST .../profile/gc` (needs `--expose-gc`). Default off — leave disabled in production.          |
...
# Long-running memory stability soak (opt-in PROFILE memory probe; default 5 min)
pnpm test:perf:memory         # writes test/perf/last-memory-{results.json,report.md}
...
- `SESSION_CACHE_TTL_MS=5000` — reuse Couch `/_session` principals for 5s (role/`_admin` revocation can lag by up to that window; set `0` for immediate re-resolve)
...
```bash
# Immediate session re-resolve (disables the 5s principal cache):
# SESSION_CACHE_TTL_MS=0 pnpm test:perf:profile
```
